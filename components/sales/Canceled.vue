<template>
  <div class="contentBody">
    ...
    >
      ...
      <template slot="order_date" slot-scope="text">{{ text | dateFormat('date') }}</template>
      <template slot="canceled_date" slot-scope="text">{{ text | dateFormat('date') }}</template>
    </a-table>
  </div>
</template>
...
const columns = [
  ...
  ,
  {
    ...
    sorter: true,
    filterMultiple: false,
    dataIndex: 'cancelled_datetime',
    scopedSlots: { customRender: 'canceled_date' }
  },
...
export default {
  ...
  computed: {
    ...
    ...mapState('cancelled', ['list', 'page', 'loading', 'count', 'limit']),
  ...
  methods: {
    ...mapActions('cancelled', ['fetchList']),
    doSearch() {
    ...
    async handleFetch(params) {
      try {
        params.url = '/sales/order/cancelled/'
        this.$store.dispatch('user/loading', true)
